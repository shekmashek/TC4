...
const BudgetTracker = () => {
  ...
  const { projects, categories, settings, allComments, vatRegimes, taxConfigs, loans } = dataState;
  const { activeProjectId, timeUnit, horizonLength, periodOffset, activeQuickSelect, quickFilter: globalQuickFilter } = uiState;
  
  const [tableauMode, setTableauMode] = useState('lecture');
  const [isPeriodMenuOpen, setIsPeriodMenuOpen] = useState(false);
  const periodMenuRef = useRef(null);
  const [quickFilter, setQuickFilter] = useState(globalQuickFilter || 'all');

  useEffect(() => {
    if (globalQuickFilter) {
      setQuickFilter(globalQuickFilter);
      uiDispatch({ type: 'SET_QUICK_PERIOD_FILTER', payload: null }); // Reset global filter after applying
    }
  }, [globalQuickFilter, uiDispatch]);
  ...
  const filteredBudgetEntries = useMemo(() => {
    ...
    if (quickFilter !== 'all') {
        if (quickFilter === 'provisions') {
            ...
            const borrowingLoanIds = new Set((loans || []).filter(l => l.type === 'borrowing').map(l => l.id));
            entries = entries.filter(e => e.loanId && borrowingLoanIds.has(e.loanId));
        } else if (quickFilter === 'lendings') {
            const lendingLoanIds = new Set((loans || []).filter(l => l.type === 'lending').map(l => l.id));
            entries = entries.filter(e => e.loanId && lendingLoanIds.has(e.loanId));
    ...
    return (
      <>
        ...
        <tr className="bg-gray-200 border-y-2 border-gray-300 cursor-pointer" onClick={toggleMainCollapse}>
          <td colSpan={(isConsolidated || isCustomConsolidated) ? 3 : 2} className="px-4 py-1 text-text-primary bg-gray-200 sticky left-0 z-10">
          ...
          return (
            <React.Fragment key={mainCategory.id}>
              <tr onClick={() => toggleCollapse(mainCategory.id)} className="bg-gray-100 text-gray-700 cursor-pointer hover:bg-gray-200">
                <td colSpan={(isConsolidated || isCustomConsolidated) ? 3 : 2} className="px-4 py-1 sticky left-0 z-10 bg-gray-100">
