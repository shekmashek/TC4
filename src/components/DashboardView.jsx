...
import { Wallet, TrendingDown, HandCoins, AlertTriangle, ArrowUp, ArrowDown, BookOpen, Lock, PiggyBank, Banknote, Coins, ListChecks, Calendar, PieChart, Layers, Table, Settings } from 'lucide-react';
...
const DashboardView = () => {
  ...
  const [tempWidgetSettings, setTempWidgetSettings] = useState(null);
  ...
  const handleOpenSettings = () => {
    setTempWidgetSettings(widgetVisibility);
    setIsSettingsModalOpen(true);
  };

  const handleCloseSettings = () => {
    setIsSettingsModalOpen(false);
    setTempWidgetSettings(null);
  };

  const handleSaveWidgetSettings = () => {
    if (!activeProject || isConsolidated || !tempWidgetSettings) return;
    updateProjectSettings({ dataDispatch, uiDispatch }, {
        projectId: activeProject.id,
        newSettings: { dashboard_widgets: tempWidgetSettings }
    });
    handleCloseSettings();
  };
    ...
    <>
      <div className="p-6 max-w-full space-y-8">
        <div className="flex justify-between items-start">
            ...
            {!isConsolidated && (
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-600 bg-white border rounded-lg hover:bg-gray-100 transition-colors"
                >
                    <Settings size={16} />
                    Personnaliser
                </button>
        ...
        {widgetVisibility.kpi_cards && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-teal-100 p-3 rounded-full"><PiggyBank className="w-6 h-6 text-teal-600" /></div><div><p className="text-sm text-gray-500">Épargne</p><p className="text-2xl font-bold text-gray-800">{formatCurrency(totalSavings, settings)}</p></div></div>
            <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-indigo-100 p-3 rounded-full"><Lock className="w-6 h-6 text-indigo-600" /></div><div><p className="text-sm text-gray-500">Provisions</p><p className="text-2xl font-bold text-gray-800">{formatCurrency(totalProvisions, settings)}</p></div></div>
            <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-red-100 p-3 rounded-full"><Banknote className="w-6 h-6 text-red-600" /></div><div><p className="text-sm font-medium text-gray-500">Emprunts à rembourser</p><p className="text-2xl font-bold text-gray-800">{formatCurrency(totalBorrowings, settings)}</p></div></div>
            <div className="bg-white p-6 rounded-lg shadow-sm border flex items-start gap-4"><div className="bg-green-100 p-3 rounded-full"><Coins className="w-6 h-6 text-green-600" /></div><div><p className="text-sm font-medium text-gray-500">Prêts à recevoir</p><p className="text-2xl font-bold text-gray-800">{formatCurrency(totalLendings, settings)}</p></div></div>
          </div>
        )}
      ...
      {tempWidgetSettings && (
        <DashboardSettingsModal 
          ...
          widgetSettings={tempWidgetSettings}
          onWidgetSettingsChange={setTempWidgetSettings}
